...
<div class="login-wrapper">
    <div class="login-inner-wrapper">
        <div class="login-sec">
            <h1 style="color: {{$data['color']}};">Login</h1>
            @if (count($errors) > 0)
            ...
            <div class="login-form">
                <form class="form-signin" role="form" method="POST" action="{{ url('/system/login') }}">
                    ...
                    <h2>You are current using IP - <strong>{{$currentIp}}</strong></h2>
                    <div class="form-group">
                        <button type="submit" class="btn login-btn btn-block" style="background-color: {{$data['color']}};">Login</button>
                    </div>
                    <div class="form-group text-center">
                        <a href="{{url('/forgotpassword')}}" switch-target="#login-forgot" switch-parent="#login-form" title="Recover password">Forgot Password ?</a>
                    </div>
                </form>
            </div><!-- ends login-form -->
        </div><!-- ends login-sec -->
