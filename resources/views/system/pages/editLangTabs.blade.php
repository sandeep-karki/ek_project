<ul class="nav nav-tabs">
  ...
  <div class="tab-content" style="border:solid 1px #dddddd; border-top:none;">
    @php $b=0; @endphp
    @foreach($flags as $flag)
    @if( ($loop->first && !Session::has('defaultLang')) || ( Session::has('defaultLang') && Session::get('flag') == $flag->id ) )
    <div id="{{ucfirst($flag->short_code)}}" class="tab-pane fade in active">{{Session::forget('flag')}}
      @else
      <div id="{{ucfirst($flag->short_code)}}" class="tab-pane fade">
        ...
        <div class="panel-body">
          ...
          @include('system.pages.editTabForm',['flag'=>$flag,'pageLangData'=>$pageLangData])
          @else
          @include('system.pages.createTabForm')
          @endif
        </div>
      </div>
      @endforeach
